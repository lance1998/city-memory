...
document.addEventListener('DOMContentLoaded', () => {
  fetch('data.json')
    ...
    .then(data => {
      window.DB = data;
      if (typeof initDBSave === 'function') initDBSave();
      if (typeof window.applyPatches === 'function') window.applyPatches();
      app.init();
    })
    .catch(error => {
      console.error('Error loading data:', error);
      if (app.toast) app.toast('数据加载失败，请刷新重试');
    });
});

window.applyPatches = function() {

...
const app = {
  ...
  filterMapMarkers() {
    ...
    for (let i = 0; i < DB.memories.length; i++) {
      memoryMap.set(DB.memories[i].id, DB.memories[i]);
    }
  ...
  initFootprintMap() {
    ...
    try {
      ...
      // ⚡ Bolt Optimization: Pre-calculate a lookup Map for memories to convert the O(N*M) lookup inside the footprints loop to O(N+M), improving rendering performance.
